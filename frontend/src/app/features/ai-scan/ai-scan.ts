...
export class AiScanComponent implements OnInit {
  ...
  private mapActivityType(aiType: string): string {
    ...
    if (
      typeLower.includes('น้ำประปา') ||
      typeLower.includes('ประปา') ||
      (typeLower.includes('น้ำ') && !typeLower.includes('น้ำมัน'))
    ) {
      return 'Water';
    }
    if (
      typeLower.includes('น้ำมัน') ||
      typeLower.includes('เชื้อเพลิง') ||
      typeLower.includes('ก๊าซ') ||
      typeLower.includes('แก๊ส') ||
  ...
  private mapUnit(aiUnit: string, type: string): string {
    ...
    if (
      unitLower.includes('หน่วย') ||
      unitLower.includes('ลบ.ม') ||
      unitLower.includes('ม3') ||
  ...
  private saveToBackend(evidenceUrl?: string) {
    ...
    if (type === 'Electricity') {
      const match = this.factors.find(
        (f) => f.name.includes('Electricity') || f.name.includes('ไฟฟ้า'),
      );
      factor = match ? match.factor_value : 0.4999;
    } else if (type === 'Water') {
      const match = this.factors.find(
        (f) => f.name.includes('Water') || f.name.includes('น้ำประปา'),
      );
      factor = match ? match.factor_value : 0.337;
    } else if (type === 'Fuel') {
      const match = this.factors.find(
        (f) =>
          f.name.includes('Diesel') || f.name.includes('ดีเซล') || f.name.includes('น้ำมันดีเซล'),
      );
