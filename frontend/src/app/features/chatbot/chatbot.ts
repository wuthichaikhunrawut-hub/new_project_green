...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-slate-900 flex">
      <!-- Sidebar -->
      <div
        class="w-64 bg-slate-950 border-r border-white/10 flex flex-col flex-shrink-0 transition-all duration-300"
      >
        <div class="p-4">
          <button
            (click)="newSession()"
            class="w-full py-2 px-4 rounded-xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/20 transition-colors flex items-center justify-center gap-2"
          >
        ...
        <div class="flex-1 overflow-y-auto px-2 pb-4 space-y-1">
          <div class="text-xs font-semibold text-slate-500 uppercase tracking-wider px-3 mb-2">
            ประวัติการสนทนา
          </div>
          @for (session of sessions; track session.id) {
            <div
              class="group flex items-center justify-between rounded-lg px-3 py-2 cursor-pointer hover:bg-white/5 transition-colors"
      ...
      <!-- Main Chat Area -->
      <div
        class="flex-1 flex flex-col min-w-0 bg-gradient-to-br from-slate-900 via-emerald-950 to-slate-900"
      >
        <!-- Header -->
        <div
          class="border-b border-white/10 bg-white/5 backdrop-blur px-6 py-4 flex items-center gap-4 flex-shrink-0"
        >
          <div
            class="w-10 h-10 rounded-2xl bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center shadow-lg"
          >
        ...
        >
          ...
          @if (messages.length === 0) {
            <div class="flex flex-col items-center justify-center py-16 text-center">
              <div
                class="w-20 h-20 rounded-3xl bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center shadow-2xl mb-6"
              >
              ...
              <p class="text-slate-400 text-sm max-w-md">
                ผมช่วยตอบคำถามเกี่ยวกับ Green Office · คาร์บอน · กลยุทธ์ความยั่งยืน และอื่นๆ
                ได้เลยครับ
              </p>
              ...
              <div class="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-lg">
                @for (prompt of quickPrompts; track prompt) {
                  ...
                  >
                    <i
                      class="fa-solid fa-arrow-right text-emerald-400 mr-2 group-hover:translate-x-0.5 transition-transform"
                    ></i
                    >{{ prompt }}
                  </button>
                }
          ...
          @for (msg of messages; track msg.id) {
            <div class="flex gap-3" [class.flex-row-reverse]="msg.role === 'user'">
              ...
              >
                ...
                <div class="text-xs opacity-40 mt-1.5 text-right">
        ...
        <div class="border-t border-white/10 bg-white/5 backdrop-blur px-4 py-4 flex-shrink-0">
          <div class="flex items-end gap-3 max-w-4xl mx-auto">
            <div
              class="flex-1 rounded-2xl bg-white/10 border border-white/20 focus-within:border-emerald-500/60 transition-colors overflow-hidden"
            >
              <textarea
                [(ngModel)]="inputText"
            ...
            >
              <i
                class="fa-solid"
                [ngClass]="isTyping ? 'fa-spinner fa-spin' : 'fa-paper-plane'"
              ></i>
            </button>
          </div>
          <p class="text-center text-slate-600 text-xs mt-2">
            GreenSync AI อาจเกิดข้อผิดพลาดได้ กรุณาตรวจสอบข้อมูลสำคัญก่อนนำไปใช้
          </p>
        </div>
      </div>
    </div>
...
export class ChatbotComponent implements OnInit {
  ...
  loadSessions() {
    this.http
      .get<any[]>(`${environment.apiUrl}/gemini/sessions`, { headers: this.getHeaders() })
      .subscribe({
        next: (res) => {
          this.sessions = res;
  ...
  selectSession(id: number) {
    ...
    this.messages = [];
    this.http
      .get<any[]>(`${environment.apiUrl}/gemini/sessions/${id}/messages`, {
  ...
  deleteSession(id: number, event: Event) {
    ...
    this.http
      .delete(`${environment.apiUrl}/gemini/sessions/${id}`, { headers: this.getHeaders() })
      .subscribe({
        next: () => {
  ...
  sendMessage() {
    ...
    this.http
      .post<{ reply: string }>(`${environment.apiUrl}/gemini/chat`, payload, { headers })
      .subscribe({
        next: (res) => {
