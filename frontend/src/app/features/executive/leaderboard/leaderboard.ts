    ...
    forkJoin({
      ...
      next: ({ dashboard, leaderboard }) => {
        ...
        this.leaderboardData = (leaderboard || []).map((item) => {
          const score = Number(item.assessmentScore ?? 0);
          return {
            rank: item.rank,
            ...
            auditScore: score,
            certifiedLevel:
              score >= 80 ? 'ดีเยี่ยม (ทอง)' : score >= 70 ? 'ดีมาก (เงิน)' : 'ดี (ทองแดง)',
            badgeClass: score >= 80 ? 'gold' : score >= 70 ? 'silver' : 'bronze',
            trend:
              item.reductionPercent > 10 ? 'up' : item.reductionPercent < 0 ? 'down' : 'neutral',
          };
        });
