...
export class PaymentController {
  ...
  async createSetupIntent(@Req() req) {
    try {
      ...
      console.log('Calling Stripe to create SetupIntent for:', stripeCustomerId);
  ...
  async listPaymentMethods(@Req() req) {
    const org = await this.subscriptionsService.getOrganizationByUserId(req.user.userId);
