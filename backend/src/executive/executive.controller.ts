...
export class ExecutiveController {
  ...
  async exportDashboardPdf(
    ...
    try {
      const data = await this.executiveService.getDashboard(orgId, {
        startDate,
        endDate,
        branchId,
      });
      ...
      if (data.carbonByScope && data.carbonByScope.length > 0) {
        data.carbonByScope.forEach((row: any) => {
          carbonRows.push([
            `Scope ${row.scope}`,
            `${row.year}`,
            Number(row.totalEmission || 0).toLocaleString('th-TH', {
      ...
      const docDefinition: any = {
        ...
        content: [
          ...
          {
            text: 'Executive Dashboard Report — Green Sync',
            style: 'subheaderEn',
            alignment: 'center',
            margin: [0, 0, 0, 6],
          },
          {
            text: `สร้างเมื่อ: ${generatedDate}`,
            style: 'label',
            alignment: 'center',
            margin: [0, 0, 0, 24],
          },
          ...
          },
          {
            table: {
              ...
              body: [
                [
                  { text: 'ชื่อองค์กร', style: 'tableHeader' },
                  data.orgName || '-',
                ],
                [{ text: 'รหัสองค์กร', style: 'tableHeader' }, `${orgId}`],
                [
                  { text: 'เป้าหมายลดคาร์บอน', style: 'tableHeader' },
                  `${data.targetReductionPercent ?? 0}%`,
                ],
                [
                  { text: 'ความคืบหน้า Net Zero', style: 'tableHeader' },
                  `${data.netZeroProgressPercent ?? 0}%`,
                ],
              ],
            },
          ...
          // Assessment stats
          {
            text: '📊 สถิติการประเมิน Green Office',
            style: 'subheader',
            margin: [0, 0, 0, 10],
          },
          {
            table: {
              ...
              body: [
                [
                  { text: 'จำนวนครั้งที่ผ่านการรับรอง', style: 'tableHeader' },
                  `${data.approvedCount ?? 0} ครั้ง`,
                ],
                [
                  { text: 'คะแนนเฉลี่ย', style: 'tableHeader' },
                  `${data.avgApprovedScore ?? 0} คะแนน`,
                ],
                [
                  { text: 'ระดับรับรองล่าสุด', style: 'tableHeader' },
                  data.latestCertifiedLevel || 'ยังไม่ได้รับรอง',
                ],
              ],
            },
            layout: 'lightHorizontalLines',
          ...
          // Carbon data
          {
            text: '🌍 ข้อมูลการปล่อยก๊าซเรือนกระจก',
            style: 'subheader',
            margin: [0, 0, 0, 10],
          },
          {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="executive_report_${orgId}.pdf"`,
      );
  ...
  setGoal(
    @Request() req: { user: JwtUser },
    @Body() body: { targetReductionPercent: number; year: number },
  ) {
    const orgId = Number(req.user.orgId ?? 0);
    if (!orgId) throw new BadRequestException('Organization ID missing');
    return this.executiveService.setGoal(
      orgId,
  ...
  getLeaderboard(
    ...
    const orgId = Number(req.user.orgId ?? 0);
    if (!orgId) throw new BadRequestException('บัญชีนี้ไม่ได้เชื่อมกับองค์กร');
    return this.executiveService.getLeaderboard(
      orgId,
